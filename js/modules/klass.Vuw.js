...
* 1.0.2: vuw-templateのidがrendar()で渡される$elmに継承される様に変更
* 1.0.3: useVuwer関数の廃止
* 1.0.4: onChangeState()で渡されるstateがthis.stateと≠となるように変更
* 1.0.5: remove時に、remove対象のVuwにisRemoved: trueを付与するように変更
*/
;(function (global, factory) {
	"use strict";
	global.vuwer = factory(global.Klass, global.jQuery || global.$);
}(this, function(Klass, $) {
	"use strict";
	...
	Klass.create("Vuw", {
		...
		* @param {string | object} k: objectの場合は自身にマージされる
			* @param {everything v: kがstringの場合、値として登録される
		* @param (optional) {boolean} isOverride: true->既に存在していた場合に上書きを行う
		* @return this
		* @note ※第１引数：kがobjectの場合、第２引数をisOverrideとみなします
		*/
		...
		*/
		...
		/**
		* テンプレート退避領域の取得
		* @param (optional) {boolean} as$: falseの場合にHTMLElementとして返す
		...
		*/
		getReady: function() {
			...
			if(this.isGettingReady) {
				...
				.then( (function() {
					var
						args = this.getAsArray(arguments),
						d = $.Deferred();
					...
					$.when(
						...
						if(arguments.length && arguments[0] !== undefined) {
							args = args.concat( this.getAsArray(arguments) );
						}
				...
				.then( (function() {
					...
					df.resolve.apply( df, this.getAsArray(arguments) );
		...
		*/
		changeState: function(state) {
			...
			.then( (function() {
				var renderedElm;
				this.state = state;
				renderedElm = this._execRender( this.getAsArray(arguments) );
				this.isChangingState = false;
				df.resolve(renderedElm);
			}).bind(this), function() {
				this.isChangingState = false;
				df.reject.apply( df, this.getAsArray(arguments) );
			});
			return df.promise();
		},
	...
	return (function() {
		var vuwer = Klass.new_("Vuw", {
			...
			_getComponentsReady: function() {
				...
				$.when.apply($, methods).then( (function() {
					...
					this.getAsArray(arguments).forEach(function(res, i) {
			...
			* @param (optional) {object | Vuw instance} opt: Vuwコンストラクタのオプション | childKlassで指定したKlassのインスタンス
			* @param (optional) {function} callback: 追加したVuwに対する処理（this = 追加したVuw）※非同期非対応
			* @param (optional) {boolean} isOverride: true->既に存在していた場合に上書きを行う
			* @return this
			*/
			...
			* @param (optional) {object} prop 継承先のプロパティ
			* @param (optional) {string | function} parent: 継承元の名称 | 継承元Class
			* @return this
			*/
		...
		*************************************/
		...
		.appendKlass("VuwerComponent", {
			...
			add: function(name) {
				vuwer.add.apply( this, this.getAsArray(arguments) );
				return this;
			},
			get: function(name) {
		...
		******************************************/
			...
			_initialize: function(opt) {
				if( Klass("VuwFade").parent._initialize.call(this, opt) ) {
					...
					this.onChangeState(function(state) {
						var toVisible = !!state.visible;
						if(toVisible !== this.state.visible) {
		...
		csl.log.gray("** vuwer (Vuw version:" + Klass("Vuw").getVersion() + ") **");
		return vuwer;
	})();
}));
